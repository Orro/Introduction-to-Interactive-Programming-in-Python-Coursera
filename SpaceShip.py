...
# 
# A 2D space inspired by the classic arcade game Asteroids (1979).
#
# 30/09/2013
...
def click(pos):
    global started, lives, score, sound
...
def draw(canvas):
    ...
    # update lives, score and soundtrack when restarting the game
...
def rock_spawner():
    global rock_group, started, increase_vel
    
    if started:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [increase_vel + random.random() * .6 - .3, increase_vel + random.random() * .6 - .3]
...
def group_collide(group, other_object):
    ...
    for element in set(group):
        if element.collide(other_object):
            collision = True
            a_explosion = Sprite(element.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
